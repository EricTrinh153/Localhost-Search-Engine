...
class FileSearch
  ...
  def check   
    #loading each and every file in the root and subfolders to an array
    file_paths = []
    Find.find(@path) do |path|
    file_paths << path if path =~ /.*\..*$/
    end
    
    #going through each and every loaded file to find a match
    file_paths.each do |file|
      #using the 'searching in file' method from the loaded gem to search valid file types and the term
      result = SearchInFile.search(file,@term)
